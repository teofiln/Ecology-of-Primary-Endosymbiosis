...
```{r, fig.width=8, fig.height=10, fig.cap="Ancestral state reconstruction for the Cyanobacteria + Archaeplastida phylogeny under an equal rates model (left), unequal rates model (middle) and with model-averaged transition rates (right). Accounting for the asymmetry in transition rates clearly favors a marine, rather than freshwater, ancestry for the MRCA of cyanobacteria and archaeplastids."}
pie.size <- c(1.5, rep(0.6, 5), 1.5, rep(0.6, 111))
par(mfrow=c(1,3), mar=c(0.5, 0.5, 2, 0.5), lwd=0.5, lend=2)
plotRECON(phy = tree2, likelihoods = ER.unord$states, piecolors = Cols, pie.cex = pie.size, font=1)
nodelabels(node = 119+c(1,6,7), adj=c(1.1,-1), bg="white", 
           text = c("R", "G+A", "A"))
title("Equal rates", adj=0, line=0)
plotRECON(phy = tree2, likelihoods = ARD.unord$states, piecolors = Cols, pie.cex = pie.size, font=1)
nodelabels(node = 119+c(1,6,7), adj=c(1.1,-1), bg="white", 
           text = c("R", "G+A", "A"))
title("All-rates-different", adj=0, line=0)
plotRECON(phy = tree2, likelihoods = mod.ave.recon$states, piecolors = Cols, pie.cex = pie.size, font=1)
nodelabels(node = 119+c(1,6,7), adj=c(1.1,-1), bg="white", 
           text = c("R", "G+A", "A"))
